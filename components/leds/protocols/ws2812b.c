...
int leds_protocol_ws2812b_init(union leds_interface_state *interface, const struct leds_options *options)
{
  ...
  switch (options->interface) {
    ...
    case LEDS_INTERFACE_UART:
      if ((err = leds_interface_uart_init(&interface->uart, &options->uart, LEDS_PROTOCOL_WS2812B_INTERFACE_UART_MODE, LEDS_INTERFACE_UART_FUNC(uart_mode_24B3I7, leds_protocol_ws2812b_uart_out)))) {
...
#endif
#if CONFIG_LEDS_UART_ENABLED
  .uart_interface_mode = LEDS_PROTOCOL_WS2812B_INTERFACE_UART_MODE,
#endif
